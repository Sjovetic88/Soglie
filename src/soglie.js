...
async function handleRequest(request, env) {
  ...
  const htmlComponenti = [
    ...
    ".riga-card .valore-destra { font-size: 16px; display: flex; align-items: center; gap: 14px; }",
    ".badge-stato { font-size: 18px; line-height: 1; display: inline-block; width: 22px; text-align: center; }",
    ...
    ".lista-partite-scroll { max-height: 250px; overflow-y: auto; background: #000000; border: 1px solid #1a1a1a; border-radius: 6px; display: none; margin-top: 15px; }",
    ...
    "</div>",
    "<p style='font-size: 13px; color: #6b7280; margin-bottom: 12px; text-align: center;'>💡 Clicca su un campionato per esaminare semafori, soglie attive e match storici.</p>",
    "<div id='lista-campionati'>",
    "</div>",
    "<div id='pannello-dettaglio-unificato' class='dettaglio-partite-container'>",
    "<h3 id='titolo-dettaglio-unificato'>Dettaglio Campionato</h3>",
    "<div id='griglia-esiti-soglie' class='griglia-semafori'>",
    "</div>",
    "<button id='btn-mostra-partite' class='btn-anteprima-match' onclick='toggleVistaPartite()'>📋 ANTEPRIMA PARTITE</button>",
    "<div id='scroll-partite-unificato' class='lista-partite-scroll'>",
    "<table>",
    "<thead>",
    "<tr>",
    "<th>Data</th>",
    ...
    "  return '🔴';",
    "}",
    "function renderizzaCampionati(dati) {",
    "  var container = document.getElementById('lista-campionati');",
    "  container.innerHTML = '';",
    "  dati.forEach(function(item) {",
    "    var card = document.createElement('div');",
    "    card.className = 'riga-card';",
    "    card.onclick = function() { selezionaCampionato(item.nazione, item.campionato, item.bandiera, item.stato, item.stato_semafori, item.stato_soglie); };",
    ...
    "    var dettMatch = document.createElement('div');",
    "    dettMatch.className = 'dettaglio-date';",
    "    dettMatch.textContent = 'Match: ' + (item.match_elaborati || 0);",
    "    infoSinistra.appendChild(nomeCamp);",
    "    infoSinistra.appendChild(dettMatch);",
    ...
    "    valoreDestra.appendChild(dotSinc);",
    "    valoreDestra.appendChild(dotSem);",
    "    valoreDestra.appendChild(dotSog);",
    "    card.appendChild(infoSinistra);",
    "    card.appendChild(valoreDestra);",
    "    container.appendChild(card);",
    "  });",
    "}",
    "async function selezionaCampionato(nazione, campionato, bandiera, stato, stato_semafori, stato_soglie) {",
    "  nazioneSelezionata = nazione;",
    "  campionatoSelezionato = campionato;",
    "  bandieraSelezionata = bandiera;",
    "  var pannello = document.getElementById('pannello-dettaglio-unificato');",
    "  var titolo = document.getElementById('titolo-dettaglio-unificato');",
    "  var griglia = document.getElementById('griglia-esiti-soglie');",
    "  var btnMostraPartite = document.getElementById('btn-mostra-partite');",
    "  var scrollPartite = document.getElementById('scroll-partite-unificato');",
    "  titolo.textContent = bandiera + ' ' + campionato + ' - Analisi Soglie';",
    "  griglia.innerHTML = '<div style=\"grid-column: 1/-1; text-align:center;color:#666;\">Caricamento semafori...</div>';",
    "  scrollPartite.style.display = 'none';",
    ...
    "      }",
    "    } catch(e) {",
    "      griglia.innerHTML = '<div style=\"grid-column:1/-1;color:red;text-align:center;\">Errore caricamento semafori</div>';",
    "    }",
    "  } else {",
    "    griglia.style.display = 'none';",
    "  }",
    "  if (stato === 'COMPLETED') {",
    "    btnMostraPartite.style.display = 'block';",
    "  } else {",
    "    btnMostraPartite.style.display = 'none';",
    "  }",
    "}",
    "async function toggleVistaPartite() {",
    "  var scrollPartite = document.getElementById('scroll-partite-unificato');",
    "  var tbody = document.getElementById('tabella-partite-dettaglio');",
    "  if (scrollPartite.style.display === 'block') {",
    "    scrollPartite.style.display = 'none';",
    "    return;",
    "  }",
    "  tbody.innerHTML = '<tr><td colspan=4 style=\"text-align:center;color:#666;\">Caricamento partite...</td></tr>';",
    "  scrollPartite.style.display = 'block';",
    "  try {",
    "    var res = await fetch('/api/partite-salvate?nazione=' + encodeURIComponent(nazioneSelezionata) + '&campionato=' + encodeURIComponent(campionatoSelezionato));",
    "    if (res.ok) {",
    "      var partite = await res.json();",
    "      tbody.innerHTML = '';",
    "      if (partite.length === 0) {",
    "        tbody.innerHTML = '<tr><td colspan=4 style=\"text-align:center;\">Nessuna partita salvata</td></tr>';",
    "        return;",
    "      }",
    ...
    "    }",
    "  } catch(e) {",
    "    tbody.innerHTML = '<tr><td colspan=4 style=\"text-align:center;color:red;\">Errore di rete</td></tr>';",
    "  }",
    "}",
    ...
    "      renderizzaCampionati(campionatiInteri);",
    ...
    "        document.getElementById('stato-operazione').className = 'status-bg';",
    "        document.getElementById('pannello-dettaglio-unificato').style.display = 'none';",
    "        await aggiornaStato();",
    "      }",
    "    } catch(e) {",
